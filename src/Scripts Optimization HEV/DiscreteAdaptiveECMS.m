%% Discrete Adaptive Equivalent Consumption Minimum Strategy (DA-ECMS)
% Con la strategia Adaptive-ECMS discreta, utilizzando l'adattamento basato
% sul feedback del SoC, il fattore di equivalenza viene aggiornato periodicamente,
% in intervalli regolari di tempo, soddisfando i vincoli progettuali. Un adattamento
% discreto permette uno scostamento maggiore dal SoC di riferimento,
% quindi, un maggior utilizzo della batteria.

function [SoC_end_DAECMS,SoC_DAECMS,P_ICE_cycle_DAECMS,var_u_DAECMS,P_fuel_cycle_DAECMS,lamb_cycle_DAECMS]=DiscreteAdaptiveECMS(iter_update,...
    Kd,lambda_DAECMS,t_cycle,w_pwt,P_pwt,P_limite_MOT)
...
lambda = (lambda_DAECMS.*ones(size(t_cycle)))';
...
for j = 2:numel(t_cycle)
    ...
    if P_pwt(j) == 0
        var_u_DAECMS(1,j) = 0; % variabile di controllo ottimale (definisce la potenza erogata dall'EM)
        I_batt_cycle(1,j) = 0; % corrente istantanea batteria
        P_fuel_cycle_DAECMS(1,j) = 0; % potenza istantanea ICE reale (tenendo conto dell'efficienza)
        P_ICE_cycle_DAECMS(1,j) = 0;  % potenza istantanea ICE ideale
    ...
    elseif P_pwt(j) > 0
        ...
        var_u_DAECMS(1,j) = var_u(index_min);
        I_batt_cycle(1,j) = I_batt(index_min);
        P_fuel_cycle_DAECMS(1,j) = P_fuel(index_min);
        P_ICE_cycle_DAECMS(1,j) = P_pwt(j)-var_u_DAECMS(1,j);
        ...
        if mod(j, iter_update) == 0
            lambda(1,j) = mean(lambda(1:j-1))+Kd*(SoC_target-SoC_cycle(1,j));
        else
        lambda(1,j) = lambda(1,j-1);
        end
        ...
        var_u_DAECMS(1,j) = var_u(index_min);
        I_batt_cycle(1,j) = I_batt(index_min);
        P_fuel_cycle_DAECMS(1,j) = 0;
        P_ICE_cycle_DAECMS(1,j) = 0;
        P_ech_cycle(1,j) = P_ech(index_min); 
        eta_ICE_cycle(1,j) = 0;
        eta_EM_cycle(1,j) = eta_EM(index_min);
        eta_batt_cycle_ECMS(1,j) = eta_batt(index_min);
...
SoC_end_DAECMS = SoC_cycle(end);
%SoC ottimale
SoC_DAECMS = SoC_cycle;
lamb_cycle_DAECMS = lambda;
end

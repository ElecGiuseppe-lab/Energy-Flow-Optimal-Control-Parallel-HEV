%% Longitudinal vehicle dynamic model and energy optimization extra-urban cycle
% Si determinano i punti operativi coppia-velocitá angolare a partire
% dal ciclo di guida e, quindi, la potenza che dovrá essere fornita dal
...
vel_cycle = cat(1,vel_WLTC_full,vel_WLTC_full,vel_WLTC_full,vel_WLTC_full);
t_cycle = (1:numel(vel_cycle))';
vel_cycle_extraUrb = vel_cycle;
t_cycle_extraUrb = t_cycle;
...
[E_cycle_pos_ICE,fc_extraUrb] = ForwardMotoring(t_cycle,P_ICE,Q_lhv);
...
set(legend('Fitting Peukert (Galushkin)',Location='northeast'),'Interpreter','latex');
ylim([0 24]);
%%
...
iter_update = 15; % numero di iterazioni prima di aggiornare il co-stato
...
figure(7)
SoC_opt_extraUrb = SoC_opt;
plot(t_cycle,SoC_cycle','b',t_cycle,SoC_opt_extraUrb,'r');
hold on
yline(SoC_target,'--');
...
figure(8)
errore_extraUrb = SoC_cycle(:,end)-SoC_target;
plot(vec_lambda,errore_extraUrb,'.-b',MarkerSize=11);
hold on
errore_opt_extraUrb = SoC_opt(end)-SoC_target;
plot(lambda_opt,errore_opt_extraUrb,'.r',MarkerSize=11);
hold on
xline(lambda_opt,'--r');
...
figure(9)
plot(t_cycle,SoC_opt_extraUrb,ColorMode="manual",Color="#A2142F");
hold on
SoC_ECMS_extraUrb = SoC_ECMS;
plot(t_cycle,SoC_ECMS_extraUrb,ColorMode="manual",Color="#77AC30");
hold on
SoC_CAECMS_extraUrb = SoC_CAECMS;
plot(t_cycle,SoC_CAECMS_extraUrb,ColorMode="manual",Color="#0072BD");
hold on
SoC_DAECMS_extraUrb = SoC_DAECMS;
plot(t_cycle,SoC_DAECMS_extraUrb,ColorMode="manual",Color="#D95319");
hold on
yline(SoC_target,'--');
...
P_ICE_cycle_opt_extraUrb = P_ICE_cycle_opt;
P_EM_cycle_opt_extraUrb = var_u_opt;

P_ICE_cycle_extraUrb_ECMS = P_ICE_cycle_ECMS;
P_EM_cycle_extraUrb_ECMS = var_u_ECMS;

P_ICE_cycle_extraUrb_CAECMS = P_ICE_cycle_CAECMS;
P_EM_cycle_extraUrb_CAECMS = var_u_CAECMS;

P_ICE_cycle_extraUrb_DAECMS = P_ICE_cycle_DAECMS;
P_EM_cycle_extraUrb_DAECMS = var_u_DAECMS;
...
fc_extraUrb_opt = E_ice_cycle/Q_lhv;
fc_extraUrb_ECMS = E_ice_cycle_ECMS/Q_lhv;
fc_extraUrb_CAECMS = E_ice_cycle_CAECMS/Q_lhv;
fc_extraUrb_DAECMS = E_ice_cycle_DAECMS/Q_lhv;

% Risparmio di carburante al traguardo PMP, ECMS, CA-ECMS
fc_sav_extraUrb = (fc_extraUrb(end)-fc_extraUrb_opt(end))/fc_extraUrb(end)*100;
fc_sav_extraUrb_ECMS = (fc_extraUrb(end)-fc_extraUrb_ECMS(end))/fc_extraUrb(end)*100; 
fc_sav_extraUrb_CAECMS = (fc_extraUrb(end)-fc_extraUrb_CAECMS(end))/fc_extraUrb(end)*100;
fc_sav_extraUrb_DAECMS = (fc_extraUrb(end)-fc_extraUrb_DAECMS(end))/fc_extraUrb(end)*100;
%%

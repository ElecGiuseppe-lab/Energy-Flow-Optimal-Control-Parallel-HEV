%% Continuosu Adaptive Equivalent Consumption Minimum Strategy (CA-ECMS)
% Con la strategia Adaptive-ECMS continua, utilizzando l'adattamento basato
% sul feedback del SoC, il fattore di equivalenza viene aggiornato dinamicamente
% ad ogni istante soddisfando i vincoli progettuali. Un adattamento continuo,
% tuttavia, impedisce un maggior utilizzo della batteria nell'intervallo SoC
% consentito. Ció é dovuto al fatto che, anche piccole deviazioni dal SoC
% di riferimento vengono subito corrette.

function [SoC_end_CAECMS,SoC_CAECMS,P_ICE_cycle_CAECMS,var_u_CAECMS,P_fuel_cycle_CAECMS,lamb_cycle_CAECMS]=ContinuousAdaptiveECMS(Kp,Ki,...
...
% Inizializzazione co-stato
lambda = (lambda_CAECMS.*ones(size(t_cycle)))';
...
for j = 2:numel(t_cycle)
    ...
    if P_pwt(j) == 0
        var_u_CAECMS(1,j) = 0; % variabile di controllo ottimale (definisce la potenza erogata dall'EM)
        I_batt_cycle(1,j) = 0; % corrente istantanea batteria
        P_fuel_cycle_CAECMS(1,j) = 0; % potenza istantanea ICE reale (tenendo conto dell'efficienza)
        P_ICE_cycle_CAECMS(1,j) = 0;  % potenza istantanea ICE ideale
    ...
    elseif P_pwt(j) > 0
        ...
        var_u_CAECMS(1,j) = var_u(index_min);
        I_batt_cycle(1,j) = I_batt(index_min);
        P_fuel_cycle_CAECMS(1,j) = P_fuel(index_min);
        P_ICE_cycle_CAECMS(1,j) = P_pwt(j)-var_u_CAECMS(1,j);
        ...
        deltaSoC(1,j) = deltaSoC(1,j-1) + (SoC_target-SoC_cycle(1,j));
        ...
        lambda(1,j) = lambda(1,1) + Kp*(SoC_target-SoC_cycle(1,j)) + Ki*deltaSoC(1,j);
        ...
        var_u_CAECMS(1,j) = var_u(index_min);
        I_batt_cycle(1,j) = I_batt(index_min);
        P_fuel_cycle_CAECMS(1,j) = 0;
        P_ICE_cycle_CAECMS(1,j) = 0;
        P_ech_cycle(1,j) = P_ech(index_min); 
        eta_ICE_cycle(1,j) = 0;
        eta_EM_cycle(1,j) = eta_EM(index_min);
        eta_batt_cycle_ECMS(1,j) = eta_batt(index_min);
...
SoC_end_CAECMS = SoC_cycle(end);
%SoC ottimale
SoC_CAECMS = SoC_cycle;
lamb_cycle_CAECMS = lambda;
end

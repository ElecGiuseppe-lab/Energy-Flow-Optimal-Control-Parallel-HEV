%% Longitudinal vehicle dynamic model and energy optimization urban cycle
...
vel_cycle = cat(1,vel_FUDS,vel_SFUDS,vel_FUDS,vel_SFUDS,vel_SFUDS,vel_FUDS,vel_FUDS,vel_SFUDS,vel_FUDS,vel_FUDS);
t_cycle = (1:numel(vel_cycle))';
vel_cycle_urb = vel_cycle;
t_cycle_urb = t_cycle;
...
[val,W_ICE,T_ICE,eta_ICE_map,eta_ICE] = EfficencyICE(w_limite_ICE,T_max_ICE,w_pwt,T_pwt);
...
[Val,W,T,eta_MOT_map,eta_GEN_map,eta_EM] = EfficencyElectricMotor(T_max_EM,w_limite_MOT,w_pwt,T_pwt); 
...
set(legend('ICE', 'Efficienza',Location='bestoutside'),'Interpreter','latex');
...
set(legend('MOT','GEN', 'Efficienza',Location='bestoutside'),'Interpreter','latex');
%%

% Energia consumata in accelerazione [J]
[E_cycle_pos_ICE,fc_urb] = ForwardMotoring(t_cycle,P_ICE,Q_lhv);
...
hold on
plot(t_cycle,abs(E_cycle_neg_GEN)/10^6,'g',LineStyle='-');
set(ylabel('Energia [MJ]'),'Interpreter','latex');
yyaxis right
plot(t_cycle,fc_urb,'m');
set(xlabel('Time/sec'),'Interpreter','latex');
set(ylabel('Consumo di carburante [lt]'),'Interpreter','latex');
set(legend('Energia consumata solo ICE','Energia recuperata con frenata rigenerativa',...
...
Kp = 4;
Ki = 2; % coefficiente di proporzionalitá integrativo
...
iter_update = 60; % numero di iterazioni prima di aggiornare il co-stato
Kd = 3; % coefficiente di proporzionalitá
...
figure(7)
SoC_opt_urb = SoC_opt;
plot(t_cycle,SoC_cycle','b',t_cycle,SoC_opt_urb,'r');
hold on
yline(SoC_target,'--',LineWidth=2);
set(ylabel('SoC'),'Interpreter','latex');
set(xlabel('Time/sec'),'Interpreter','latex');

% Plot effetto del valore iniziale del co-stato sul SoC finale
figure(8)
errore_urb = SoC_cycle(:,end)-SoC_target;
plot(vec_lambda,errore_urb,'.-b',MarkerSize=11);
hold on
errore_opt_urb = SoC_opt(end)-SoC_target;
plot(lambda_opt,errore_opt_urb,'.r',MarkerSize=11);
hold on
xline(lambda_opt,'--r');
set(xlabel('${\lambda}_0$'),'Interpreter','latex');
set(ylabel('Errore (${\Delta}SoC$)'),'Interpreter','latex');
yline(0,'--');
text(vec_lambda(1,1)+0.01,0.02,'$SoC_{end}$ = $SoC_{target}$',Interpreter='latex')
%%

% Plot traiettoria SoC ottimale PMP vs ECMS vs CA-ECMS vs DA-ECMS
figure(9)
subplot(3,1,1)
plot(t_cycle,SoC_opt_urb,ColorMode="manual",Color="#A2142F");
hold on
SoC_ECMS_urb = SoC_ECMS;
plot(t_cycle,SoC_ECMS_urb,ColorMode="manual",Color="#77AC30");
hold on
SoC_CAECMS_urb = SoC_CAECMS;
plot(t_cycle,SoC_CAECMS_urb,ColorMode="manual",Color="#0072BD");
hold on
SoC_DAECMS_urb = SoC_DAECMS;
plot(t_cycle,SoC_DAECMS_urb,ColorMode="manual",Color="#D95319");
hold on
yline(SoC_target,'--',LineWidth=2);
...
%Plot variazione co-stato e fattore di equivalenza
subplot(3,1,2)
plot(t_cycle,lamb_cycle_opt);
hold on
plot(t_cycle,lamb_cycle_ECMS);
hold on
plot(t_cycle,lamb_cycle_DAECMS);
set(ylabel('Co-state or EF'),'Interpreter','latex');
set(xlabel('Time/sec'),'Interpreter','latex');
set(legend('PMP','ECMS','Discrete A-ECMS',Location='bestoutside'),'Interpreter','latex');
ylim([lamb_cycle_DAECMS(1) 3.1]);
subplot(3,1,3)
plot(t_cycle,lamb_cycle_CAECMS);
set(ylabel('EF'),'Interpreter','latex');
set(xlabel('Time/sec'),'Interpreter','latex');
set(legend('Continuous A-ECMS',Location='bestoutside'),'Interpreter','latex');
set(gcf,'Position',[400 0 1000 800]);
%%

% Punti operativi EM e ICE (PMP, ECMS, CA-ECMS, DA-ECMS)
P_ICE_cycle_opt_urb = P_ICE_cycle_opt;
P_EM_cycle_opt_urb = var_u_opt;

P_ICE_cycle_urb_ECMS = P_ICE_cycle_ECMS;
P_EM_cycle_urb_ECMS = var_u_ECMS;

P_ICE_cycle_urb_CAECMS = P_ICE_cycle_CAECMS;
P_EM_cycle_urb_CAECMS = var_u_CAECMS;

P_ICE_cycle_urb_DAECMS = P_ICE_cycle_DAECMS;
P_EM_cycle_urb_DAECMS = var_u_DAECMS;
...
% Plot punti operativi EM e ICE post-ottimizzazione
figure(10)
...
E_ice_cycle_opt = zeros(1,numel(t_cycle));
...
for j=1:numel(t_cycle)-1
    E_ice_cycle_opt(j+1) = E_ice_cycle_opt(j) + dt_cycle*P_fuel_cycle_opt(j);
...
fc_urb_opt = E_ice_cycle_opt/Q_lhv;
fc_urb_ECMS = E_ice_cycle_ECMS/Q_lhv;
fc_urb_CAECMS = E_ice_cycle_CAECMS/Q_lhv;
fc_urb_DAECMS = E_ice_cycle_DAECMS/Q_lhv;


% Risparmio di carburante al traguardo PMP, ECMS, CA-ECMS
fc_sav_urb = (fc_urb(end)-fc_urb_opt(end))/fc_urb(end)*100;
fc_sav_urb_ECMS = (fc_urb(end)-fc_urb_ECMS(end))/fc_urb(end)*100;
fc_sav_urb_CAECMS = (fc_urb(end)-fc_urb_CAECMS(end))/fc_urb(end)*100;
fc_sav_urb_DAECMS = (fc_urb(end)-fc_urb_DAECMS(end))/fc_urb(end)*100;
%%
